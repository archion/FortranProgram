program main
	implicit none
	real(8),parameter :: t=0.25d0,tp=-0.025d0,tpp=0.02d0,tppp=0d0,tiv=0d0,nf=0.85d0,&
	...
	real(8) :: eka,eks,e1(2),e2(2),e(4),kx,ky,n,US,USp,sp=-0.2,sa,sb,sp0,dk,dt(2),dtp(2),gk(2),hn(4),wide,tmp,tmp1,tmpa,z,th,gap,&
		u2(2),v2(2),uv(2),cos2,sin2,sc,h(4,4),work(15),gm(4,2),rtmp(4,4,4,2),f(4),R(2),omg
	...
	do p=1,1,1
		...
		do l=0,500
			omg=l*0.001d0
			R=0d0
			!$OMP PARALLEL DO REDUCTION(+:R) PRIVATE(kx,ky,gk,dk,gm,h,work,e,info,f,rtmp) SCHEDULE(GUIDED)
			do i=0,mp1
				kx=pi/mp1*i
				do j=0,mp1-i
					ky=pi/mp1*j
			...
			enddo
			!$OMP END PARALLEL DO
			write(50,"(3e16.3)")omg,2d0*R/mp1**2
		enddo
	enddo
	close(10)
	close(20)
