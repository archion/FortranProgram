...
program main
	...
	real(8), allocatable :: U(:,:),V(:,:),X(:,:),iX(:,:),Y(:,:),iY(:,:),H(:,:)
	real(8) :: vec(chi*chi),ld(0:1,chi),Hexp(d**2,d**2),HU(size(Hexp,1),size(Hexp,2)),E
	...
	do i=0,N-1
		...
		call svd_a(matmul(matmul(Y,diag(ld(i1,:))),X),U,s,V)
