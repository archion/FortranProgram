...
PROGRAM CPE
	USE GLOBAL
	IMPLICIT NONE
	INTEGER,PARAMETER :: COF=2048
	INTEGER :: i,j,k,l,JA(2*DN*DN*13),IA(2*DN*DN+1),NP(4),n,n_MOD(3),INFO,Im(5)=-1
	REAL(8) :: nd(DN*DN,0:1),DT(DN*DN,4),VA(2*DN*DN*13),x,W,sp,hhn(0:2,2*DN*DN),P(0:2),EMAX,EMIN,a,b,An(0:2*COF-1),DOS(0:2*COF-1)
	REAL(8) :: E(2*DN*DN),H(2*DN*DN,2*DN*DN),WORK(3*2*DN*DN),SHD,LD=0.001
	OPEN(UNIT=10,FILE="../DATA/output.dat")
	PBC=(/DN,(i,i=1,DN),1/)
	DO i=1,DN*DN
		DT(i,:)=(/-0.2D0,0.2D0,-0.2D0,0.2D0/)
	ENDDO
	sp=0
	nd(:,0)=nf0/2
	nd(:,1)=nf0/2
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	DOS=0
	SHD=SINH(LD)
	!$OMP PARALLEL DO REDUCTION(+:DOS) PRIVATE(hhn,An,n_MOD) NUM_THREADS(2)
	DO i=1,2*DN*DN
		hhn=0
		hhn(0,i)=1D0
		An(0)=hhn(0,i)
		CALL CRSMV(VA,JA,IA,hhn(0,:),hhn(1,:),2*DN*DN)
		An(1)=hhn(1,i)*SINH(LD*(1.0-1.0/COF))/SHD
		DO n=2,COF-1
			n_MOD(1)=MOD(n-2,3)
			n_MOD(2)=MOD(n-1,3)
			n_MOD(3)=MOD(n,3)
			...
			hhn(n_MOD(3),:)=2*hhn(n_MOD(3),:)-hhn(n_MOD(1),:)
			An(n)=hhn(n_MOD(3),i)*SINH(LD*(1.0-REAL(n)/COF))/SHD
		ENDDO
		An(COF:2*COF-1)=0
	...
	ENDDO
	!$OMP END PARALLEL DO
	WRITE(10,"(2E16.3)")(a*COS(PI*(i+0.5)/(2*COF-1))+b,DOS(i),i=0,2*COF-1)
	! DO i=1,DN*DN
		...
		! ENDDO
		! ! on site
		! H(2*i-1:2*i,2*i-1:2*i)=H(2*i-1:2*i,2*i-1:2*i)+RESHAPE((/DU*nd(i,1)-sp,0.0D0,0.0D0,-DU*nd(i,0)+sp/),&
			! (/2,2/))
		! IF(ANY(Im==i)) THEN
	...
	! DO i=1,2*DN*DN
		! j=j+1
		! IF(ABS(E(i)-E(i+1))>1E-3) THEN
			! WRITE(10,"(E16.3I5)")E(i),j
			! j=0
		! ENDIF
	! ENDDO
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
