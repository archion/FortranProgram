...
program main
	...
	f=openfile(unit=110,file='../data/peak.dat')
	...
	do l=0,ubound(n,1),2
		nf=n(l)
		Tk=Tc(1)
		Tc(1)=find_order(2,-1,Tk,(/1d-4,0.25d0,5d-3/),1d-3)
		Tk=Tc(2)
		Tc(2)=find_order(2,1,Tk,(/1d-4,Tc(1),5d-3/),1d-3)
	...
	do l=0,ubound(n,1),2
		nf=n(l)
		Tk=Tc(1)
		Tc(1)=find_order(3,-1,Tk,(/1d-4,0.3d0,5d-3/),1d-3)
		Tk=Tc(2)
		Tc(2)=find_order(3,1,Tk,(/1d-4,Tc(1),5d-3/),1d-3)
		if(Tc(1)<2d-4) then
			call swap(nf,pnf)
			dnf=abs(nf-pnf)
			Tc(1)=1d-1
			do
				nf=nf-dnf/10d0
				Tk=Tc(1)
				Tc(1)=find_order(3,-1,Tk,(/1d-4,0.25d0,5d-3/),1d-3)
				Tk=Tc(2)
				Tc(2)=find_order(3,1,Tk,(/1d-4,Tc(1),5d-3/),1d-3)
				write(10,"(es12.4$)")nf,Tc
				write(*,"(es12.4$)")nf,Tc
		...
		if(Tc(2)>2d-4.and.flag) then
			...
			Tk=find_order(3,-1,Tk,(/1d-4,0.25d0,5d-3/),1d-3)
