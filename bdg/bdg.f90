...
contains
	subroutine initial()
		...
		do i=iv(0)-n+sign(1,n),iv(0),sign(1,n)
			...
			tmp(i)%val=sin(sum(2d0*q*(latt%bond(tmp(i)%nb)%bd(tmp(i)%n(1))%r+(/0d0,0.5d0,0d0/))))+6d-2
			tmp(i)%val=0d-1
			do k=1,size(tmp(i)%n)
		...
		do i=iv(0)-n+sign(1,n),iv(0),sign(1,n)
			...
			tmp(i)%val=cos(sum(q*(latt%bond(tmp(i)%nb)%bd(tmp(i)%n(1))%r+(/0d0,0.5d0,0d0/))))
			tmp(i)%val=0d-2
			do k=1,size(tmp(i)%n)
		...
		do i=iv(0)-n+sign(1,n),iv(0),sign(1,n)
			...
			tmp(i)%val=sin(sum(2d0*q*(latt%bond(tmp(i)%nb)%bd(tmp(i)%n(1))%r+(/0d0,0.5d0,0d0/))))+nf/2d0
			tmp(i)%val=nf/2d0
			do k=1,size(tmp(i)%n)
				tmp(i)%bd_sg(k)=&
					1d0
	...
	subroutine do_var(n,x,v,info)
		...
		if(n>1) then
			i=i+1
			write(20,"(es17.9$)")fE,err
			write(20,"(es17.9)")var(1)%val
			if(info<0) then
				call export_data(10)
			endif
			write(*,"(i5$)"),i
