...
contains
	...
	subroutine wf_r(var,wf,dwf)
		...
		real(8) :: eg(Ns2*2),var(vn),nvar(vn)
		integer :: i,j,n,ix(2)
		H=0d0
		wf=0d0
		dwf=0d0
		do i=1,Ns2
			...
			do j=1,4
				...
				H(i,neb(i,j,1)+Ns2)=var(1)*(-1)**mod(j,2)
				H(i+Ns2,neb(i,j,1))=var(1)*(-1)**mod(j,2)
			enddo
			! on site
		...
		do i=1,Ns2
			do j=1,Ns2
				...
				!enddo
				!wf(i,j)=wf(i,j)+sum(D(i,1:Ns2)*D(1:Ns2,Ns2+j)
				wf(i,j)=wf(i,j)+sum(&
					D(i,1:Ns2)*D(1:Ns2,Ns2+j)&
					+&
					D(i+Ns2,1:Ns2)*D(Ns2+1:Ns2*2,Ns2+j)&
					)
			enddo
		enddo
		...
		do n=1,vn
			...
			do i=1,Ns2
				call latt_one2two(i,Ns,ix)
				do j=1,4
					H(i,neb(i,j,1))=-t(1)+nvar(4)*img*(-1)**mod(sum(ix),2)*(-1)**mod(j,2)
					H(i+Ns2,neb(i,j,1)+Ns2)=-H(i,neb(i,j,1))
					H(i,neb(i,j,2))=-nvar(5)
					H(i+Ns2,neb(i,j,2)+Ns2)=-H(i,neb(i,j,2))
					H(i,neb(i,j,3))=-nvar(6)
					H(i+Ns2,neb(i,j,3)+Ns2)=-H(i,neb(i,j,3))
					H(i,neb(i,j,1)+Ns2)=nvar(1)
					H(i+Ns2,neb(i,j,1))=nvar(1)
				enddo
				! on site
				H(i,i)=nvar(3)*(-1)**mod(sum(ix),2)-nvar(2)
				H(i+Ns2,i+Ns2)=nvar(3)*(-1)**mod(sum(ix),2)+nvar(2)
			enddo
			call heevd(H,eg,"V")
			D(1:Ns2,1:Ns2)=transpose(conjg(H(1:Ns2,Ns2+1:Ns2*2)))
			D(Ns2+1:2*Ns2,1:Ns2)=-transpose((H(Ns2+1:2*Ns2,1:Ns2)))
