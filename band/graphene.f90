module pmt
	...
	use M_hamilton
	implicit none
	real(8), parameter :: t(2)=(/1d0,-0.3d0/),phi=1d0,dm=0d0
contains
	subroutine initial()
		...
		! lattice 
		latt%a1=(/3d0,sqrt(3d0),0d0/)/2d0
		latt%a2=(/3d0,-sqrt(3d0),0d0/)/2d0
		latt%T1=latt%a1
		latt%T2=latt%a2
		latt%bdc(1)=1d0
		latt%bdc(2)=1d0
		...
		do i=iv(0)-n+sign(1,n),iv(0),sign(1,n)
			do k=1,size(tmp(i)%n)
				tmp(i)%bd_sg(k)=-1d0*ab(latt%bond(tmp(i)%nb)%bd(tmp(i)%n(k))%i(1))&
					*phase_rule(latt%bond(tmp(i)%nb)%bd(tmp(i)%n(k))%dir)&
					*img*phi
			enddo
			tmp(i)%val=t(2)
		enddo
	...
	function phase_rule(dir)
		real(8) :: dir(:),er,phase_rule
		integer :: i
		phase_rule=1d0
		do i=0,2
			if(0d0-1d-6+pi*2d0/3d0*i<theta(dir).and.theta(dir)<=pi/3d0-1d-6+pi*2d0/3d0*i) then
				phase_rule=1d0
			endif
		enddo
	end function
