...
contains
	function dwave(i,sb,nb,th)
		integer, intent(in) :: i
		integer, optional, intent(in) :: sb(2),nb
		...
		dwave=cos((theta(latt%sb(o_sb(1),o_sb(2))%nb(o_nb)%bd(i)%dr)-o_th)*2d0)
	end function
	...
	subroutine gen_var(sg,nb,sb,val,V,Vnb)
		...
		if(present(val)) then
			...
			allocate(var(iv(0))%bd2v(size(latt%sb(var(iv(0))%sb(1),var(iv(0))%sb(2))%nb(nb)%bd)))
	...
	subroutine Hamilton(self,H,k,fn,info)
		...
		Ns=latt%Ns
		do l=1,size(self)
			do n=1,size(latt%sb(self(l)%sb(1),self(l)%sb(2))%nb(self(l)%Vnb)%bd)
				i=latt%sb(self(l)%sb(1),self(l)%sb(2))%nb(self(l)%Vnb)%bd(n)%i(1)
				j=latt%sb(self(l)%sb(1),self(l)%sb(2))%nb(self(l)%Vnb)%bd(n)%i(2)
				dr=latt%sb(self(l)%sb(1),self(l)%sb(2))%nb(self(l)%Vnb)%bd(n)%dr
				if(self(l)%is_V) then
					bdc=1d0
				else
					bdc=latt%sb(self(l)%sb(1),self(l)%sb(2))%nb(self(l)%Vnb)%bd(n)%bdc
				endif
	...
	subroutine  MF_val(ll,v,fE,info)
		...
		if(is_sc) then
			do l=lbound(var,1),ubound(var,1)
				if(var(l)%nb==0) then
					do n=1,size(latt%sb(var(l)%sb(1),var(l)%sb(2))%nb(var(l)%Vnb)%bd)
						i=latt%sb(var(l)%sb(1),var(l)%sb(2))%nb(var(l)%Vnb)%bd(n)%i(1)
						j=latt%sb(var(l)%sb(1),var(l)%sb(2))%nb(var(l)%Vnb)%bd(n)%i(2)
						do ii=1,2
