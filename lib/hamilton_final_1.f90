...
module M_Hamilton_final_M
	...
	type t_var
		...
		real(wp), allocatable :: sg(:)
		logical, allocatable :: cg(:)
		real(wp) :: V
		real(wp), allocatable :: val(:)
		complex(wp), allocatable :: bd(:)
		integer, allocatable :: bd2v(:)
		integer :: n
		character(:), allocatable :: label
		real(wp), allocatable :: extdat(:)
	end type
	type t_ham
		type(t_var), allocatable :: var(:)
		complex(wp), allocatable :: Uqi(:,:),Uik(:,:)
	...
	!procedure(real(wp)), pointer :: free_energy
...
contains
	function add(self,nb,ca,n,cg,sg,val,V,label,is_var,extdat) result(idx)
		...
		real(wp), intent(in), optional :: extdat(:)
		integer :: idx
	...
	function put(self,idx)
		...
		integer :: idx(:)
		real(wp) :: put(sum(self%var(idx)%n))
		integer :: n,l1,l2
		n=0
		do l1=1,size(idx)
			do l2=1,size(self%var(idx(l1))%val)
	...
	subroutine init(self)
		...
		associate(Ns=>latt%Ns,Nc=>latt%Nc,Ni=>latt%Ni,sb=>latt%sb,nq=>pbrizon%nq,nk=>pbrizon%nk,ptp=>self%ptp)
			if(all(abs(latt%bdc(:2))>1e-6_wp)) then
				...
				do i=1,self%Hi
					...
					enddo
				enddo
				self%Uqi=self%Uqi*(1._wp/sqrt(real(Ni/sb)))
			endif
			if(latt%is_all) then
				...
				do i=1,self%Hs
					...
					enddo
				enddo
				self%Uik=self%Uik*(1._wp/sqrt(real(Nc)))
			endif
		end associate
	end subroutine
	subroutine Hamilton(self,idx,H,k)
		...
		do l=1,size(idx)
			...
			do n=1,size(latt%nb(nb)%bd)/merge(latt%Nc,1,(present(k).and.latt%is_all))
				...
				bd=bd*self%var(il)%val(self%var(il)%bd2v(n))*merge(1._wp,self%var(il)%V,isnan(self%var(il)%V))
	...
	subroutine dHamilton(self,idx,H,cH,D,k)
		...
		do l=1,size(idx)
			...
			do n=1,size(latt%nb(nb)%bd)/merge(latt%Nc,1,(present(k).and.latt%is_all))
				...
				bd=bd*merge(1._wp,self%var(il)%V,isnan(self%var(il)%V))
	...
	!function fenergy(E,f)
		...
		!tmp=0._wp
		!do l=1,size(E)
			!if((-E(l))>1e2_wp*Tk) then
				!tmp=tmp+E(l)
			!else
				!tmp=tmp-Tk*log(1._wp+exp(-E(l)/Tk))
			!endif
		!enddo
		!fenergy=tmp
	!end function
	!function entropy(E,f)
		...
		!tmp=0._wp
		!do l=1,size(E)
			!tmp=tmp-(f(l)*log(max(f(l),1e-17_wp))+(1._wp-f(l))*log(max(1._wp-f(l),1e-17_wp)))
		!enddo
		!entropy=-tmp*Tk
	!end function
	!subroutine  MF_val(ll,v,fE,info)
		...
		!complex(wp) :: H(latt%Ns*spin,latt%Ns*spin)
		!real(wp) :: E(size(H,1)),av(sum(var%n))
		!complex(wp) :: D(size(H,1),1,size(av)),cH(size(H,1),size(H,2))
		!real(wp) :: f(size(E)),tmp,mt(size(av)),fE1,x(size(av)),px(sum(var(:0)%n))
		...
		!do k=1,size(pbrizon%k,1)
			...
			!f=1._wp/(exp(min(E/Tk,1e2_wp))+1._wp)
		...
		!do l=lbound(var,1),ubound(var,1)
				...
				!if(var(l)%nb==var(l)%Vnb) then
					...
					!else
						!do m=1,size(var(l)%bd2v)
							...
							!endif
						!enddo
						!if(abs(real(var(l)%bd(m)))>1e-10_wp) then
			...
			!x(lv+1:lv+var(l)%n)=1._wp/mt(lv+1:lv+var(l)%n)*av(lv+1:lv+var(l)%n)
		...
		!do l=lbound(var,1),0
			!do n=1,size(var(l)%bd)
				!if(abs(var(l)%val(var(l)%bd2v(n)))>1e-17_wp) then
	...
	!function default_free_energy()
		!integer :: l,n
		!real(wp) :: default_free_energy
		!default_free_energy=0._wp
		!do l=lbound(var,1),ubound(var,1)
			!if(var(l)%tp==1) then
				!cycle
			!endif
	...
	!function Green(gm,k,omg)
		!real(wp) :: k(:),gm,omg
		!complex(wp) :: H(latt%Ns*spin,latt%Ns*spin),Green
		!real(wp) :: E(size(H,1))
		!integer :: n,i,j
	...
	!subroutine DOS(ut,gm,omg,m,peak)
		!integer :: ut,m
		!real(wp) :: gm,omg(:)
		!real(wp), allocatable, optional :: peak(:)
		!complex(wp) :: H(latt%Ns*spin,latt%Ns*spin)
		!real(wp) :: E(size(H,1)),D(m),x,domg
		!integer :: k,i,l
	...
	!subroutine fermis(ut,gm,k,omg)
		!integer :: ut
		!real(wp) :: gm,omg,k(:,:)
		!complex(wp) :: A(size(k,1)),G(latt%Ns,latt%Ns),H(latt%Ns*spin,latt%Ns*spin)
		!real(wp) :: q(3),E(size(H,1))
		!integer :: i,j,l,m,nk
		!A=0._wp
		!nk=size(pbrizon%k,1)
		...
		!do m=1,nk
			!k=ki+dk*m
			!H=0._wp
			!call self%Hamilton([self%rg(1):self%rg(2)],H,pbrizon%k(m,:))
			!call mat_diag(H,E)
			!write(ut,"((es17.9)$)")k
			!do l=1,size(E)
				!G=0._wp
				!do i=1,latt%Ni
					...
					!enddo
				!enddo
				!write(ut,"((es17.9)$)")E(l),G*1._wp/pbrizon%nq
			!enddo
			!write(ut,"(x)")
		!enddo
		!do m=1,nk
	...
	!subroutine energy(ut,k)
		!integer :: ut
		!real(wp) :: k(:,:)
		!complex(wp) :: H(latt%Ns*spin,latt%Ns*spin),G(latt%Ns,latt%Ns,size(H,1)*2)
		!real(wp) :: E(size(H,1)),A(size(k,1),size(H,1)*2),q(3)
		!integer :: nk,m,l,i,j
		!A=0._wp
		!nk=size(pbrizon%k,1)
		!!$omp parallel do private(h,e,g,q)
	...
	!subroutine band_e(ut,gm,ki,kf,n,omg,m)
		!integer :: ut,n,m
		!real(wp) :: ki(3),kf(3),gm,omg(2)
		!complex(wp) :: H(latt%Ns*spin,latt%Ns*spin)
		!real(wp) :: A(n,m),dk(3),domg,E(size(H,1))
		!integer :: l1,l2,i,j
		!domg=(omg(2)-omg(1))/m
		!dk=(kf-ki)/n
		!A=0._wp
	...
	subroutine band(self,ut,ki,kf,n)
		...
		real(wp) :: k(3),dk(3)
		integer :: i,l,j,m
		complex(wp) :: H(self%Hi,self%Hi)
		real(wp) :: E(size(H,1)),G
		dk=(kf-ki)/n
		do m=0,n-1
			k=ki+dk*m
			H=0._wp
	...
	!function find_order(l,is,x,rang,z)
		!integer :: l,is
		!real(wp) :: find_order,x,rang(3)
		!real(wp) :: z
		!real(wp) :: pod,order,dx,mdx
		!integer :: isg
		!dx=rang(3)
		!mdx=dx/20._wp
		!call self_consist()
