...
contains
	...
	subroutine gen_latt(self,nb)
		...
		associate(T1=>self%T1,T2=>self%T2,a1=>self%a1,a2=>self%a2,a3=>self%a3,Ns=>self%Ns,layer=>self%layer,bdc=>self%bdc)
			...
			Ns=0
			do n=1,sb
				allocate(self%sb(n,n)%nb(0:nb))
				call gen_grid(a1,a2,self%rsb(n,:),T,tmp)
				allocate(self%sb(n,n)%nb(0)%bd(size(tmp,1)))
				do i=1,size(tmp,1)
					self%sb(n,n)%nb(0)%bd(i)%r=tmp(i,:)
					self%sb(n,n)%nb(0)%bd(i)%i=(/i,i/)
					self%sb(n,n)%nb(0)%bd(i)%dr=0d0
					self%sb(n,n)%nb(0)%bd(i)%bdc=1d0
				enddo
				Ns=Ns+size(self%sb(n,n)%nb(0)%bd)
			enddo
			!do i=2,layer
				!do j=1,size(a3)
			...
			do n1=1,sb
				do n2=n1,sb
					if(.not.allocated(self%sb(n1,n2)%nb)) allocate(self%sb(n1,n2)%nb(0:nb))
					k=0
					do i=1,size(self%sb(n1,n1)%nb(0)%bd)
						do j=1,size(self%sb(n2,n2)%nb(0)%bd)
							if(i==j.and.n1==n2) cycle
							do m1=-l1,l1
								do m2=-l2,l2
									dr=self%sb(n2,n2)%nb(0)%bd(j)%r-self%sb(n1,n1)%nb(0)%bd(i)%r+T1*m1+T2*m2
									k=k+1
									sort(k)%val=sum(dr**2)
									sort(k)%i=(/i,j/)
									sort(k)%dr=dr
					...
					o:	do m=c(l),c(l+1)-1
							do p=c(l),m-1
								if(sum(abs(sort(m)%dr(:)+sort(p)%dr(:)))<err.and.n1==n2.and.sort(m)%i(2)==sort(p)%i(1).and.sort(m)%i(1)==sort(p)%i(2)) then
									cycle o
								endif
							enddo
							k=k+1
							bd(k)%i=sort(m)%i
							bd(k)%r=self%sb(n1,n1)%nb(0)%bd(sort(m)%i(1))%r+sort(m)%dr/2d0
							bd(k)%dr=sort(m)%dr
							bd(k)%bdc=sort(m)%bdc
						enddo o
						allocate(self%sb(n1,n2)%nb(l)%bd(k))
						self%sb(n1,n2)%nb(l)%bd=bd(:k)
					enddo
					deallocate(c)
				enddo
			enddo
	...
	subroutine check_lattice(ut)
		...
		do m=1,size(latt%sb,1)
			do k=1,size(latt%sb(m,m)%nb(0)%bd)
				write(ut,"(es13.2$)")latt%sb(m,m)%nb(0)%bd(k)%r-latt%sb(m,m)%nb(0)%bd(k)%dr/2d0,latt%sb(m,m)%nb(0)%bd(k)%dr
				write(ut,"(i7$)")latt%sb(m,m)%nb(0)%bd(k)%i(1)
				write(ut,"(x)")
			enddo
		enddo
		write(ut,"(x/)")
		...
		do m=1,size(latt%sb,1)
			do n=m,size(latt%sb,1)
				do l=1,ubound(latt%sb(m,n)%nb,1)
					do k=1,size(latt%sb(m,n)%nb(l)%bd)
						write(ut,"(es13.2$)")latt%sb(m,n)%nb(l)%bd(k)%r-latt%sb(m,n)%nb(l)%bd(k)%dr/2d0,latt%sb(m,n)%nb(l)%bd(k)%dr
						write(ut,"(i7$)")k
						write(ut,"(x)")
					enddo
					write(ut,"(x/)")
