...
contains
	function dwave(i,sb,nb,th)
		integer, intent(in) :: i
		integer, optional, intent(in) :: sb,nb
		...
		dwave=cos((theta(latt%sb(o_sb)%nb(o_nb)%bd(i)%dr)-o_th)*2d0)
	end function
	...
	subroutine gen_var(sg,nb,sb,val,V,Vnb)
		...
		if(present(val)) then
			...
			allocate(var(iv(0))%bd2v(size(latt%sb(var(iv(0))%sb)%nb(nb)%bd)))
	...
	subroutine Hamilton(self,H,k,fn,info)
		...
		Ns=latt%Ns
		do l=1,size(self)
			do n=1,size(latt%sb(self(l)%sb)%nb(self(l)%Vnb)%bd)
				i=latt%sb(self(l)%sb)%nb(self(l)%Vnb)%bd(n)%i(1)
				j=latt%sb(self(l)%sb)%nb(self(l)%Vnb)%bd(n)%i(2)
				dr=latt%sb(self(l)%sb)%nb(self(l)%Vnb)%bd(n)%dr
				bdc=latt%sb(self(l)%sb)%nb(self(l)%Vnb)%bd(n)%bdc
