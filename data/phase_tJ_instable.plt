...
set output "phase_tJ_instable."."eps"
...
do for[i=0:(ix*iy-1)]{
	...
	set label word("SDW+SC DDW+SC",i+1) at graph 0.1/(gx*sx),1.-0.15/(gy*sy)
	set style fill transparent solid 0.3 border
	#set style rect fc lt -1 fs solid 0.15 noborder
	#set obj rect from 0.14, graph 0 to 0.175, graph 1 behind
	plot [0.05:0.25][0:0.05] "phase_tJ_instable.dat" index i using (1-$1):($2/2.):(1e7) smooth acsplines lc 1, "" index i u (1-$1):($3/2.):(1e8) smooth acsplines lc 3 , "" index i u (1-$1):($4/2.):(1e8) smooth acsplines lc 3 
    unset label
	unset format
}
#data
