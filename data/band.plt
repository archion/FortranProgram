...
set yrange [:]
set zrange [:]
#plot [:][:] for[j=4:32*3:2] "-" index 0 using 0:(column(j)):(column(j+1)<1e-4?1e-4:(column(j+1)>2?2:column(j+1))) with p ps var pt 7 lc 1
plot [:][:] for[j=4:100*4:2] "-" index 0 using 0:(column(j)) with p ps 0.2 pt 7 lc 1 lw 0.5
...
set xtics int(GPVAL_X_MAX-GPVAL_X_MIN+1)/2
...
if(exists("x1lb")) {
	...
	set xtics ("0" 0, "K" 100, "M" 200, "0" 299) out nomirror scale 0.7, 0.3 offset 0,0.5 font ",".(ticfontsize)
	if(exists("zlb")) {
