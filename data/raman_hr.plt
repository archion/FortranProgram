...
do for[i=0:(ix*iy-1)]{
	...
	plot [:0.32][0:0.8] for[j=0:0] "-"  index 0 every :::i::i using 1:(column(2+j)) with line lw 5 title ""#, "-" index 0 every :::i::i using 1:(column(4+j)>0?column(2+j)-0.0001:1/0)  with points pt 9 ps 0.7 lc 0,"-" index 0 every :::i::i using 1:(column(4+j)>0?column(2+j):1/0):(sprintf("{/=10 %0.3f}",column(1))) with labels notitle
	unset label
	unset format
}
#data
