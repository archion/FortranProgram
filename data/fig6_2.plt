...
set xtics 0.5 out nomirror scale 0.5 offset 0,0.5
...
do for[i=0:(ix*iy-1)]{
	...
	plot [:][0.03:0.3] for[j=0:2] "-" index 0 using (column(-1)/11.):1:(column(j+10)==1?column(2+j)<1e-2?0:column(2+j):1/0) with p pt 7 ps var lc 2-j+1 title "","-" index 1 using  (column(0)/11.):1 with line lw 4 lc 0
	unset label
	unset format
}
#data
