...
c_=0.45
do for[i=1:n] {
	#fit [x=1e-7:1e-6] b_*x**a_  "-" index i-1 every :::10-1::10-1 u (pi*column("T")/sin(pi*column("tau")*column("T"))):(-column("Gft")) via a_,b_
		a[i]=a_
		b[i]=b_
}
plot for[i=1:n] for[j=10:1:-1] "-" index i-1 every :::j-1::j-1 u (pi*column("T")/sin(pi*column("tau")*column("T"))):(-column("Gft")) with l lc j lw 3 notitle , 1.2*x**0.4 w l dt 2 lc 0 title "C'/LM'" #,5*x**(4./3.) w l dt 2 lc 1 title "MCK"
set key font ",".ticfontsize at graph 0.95,0.6,1 horizontal maxcols 1 spacing 0.9 samplen 1.5 #opaque autotitle
