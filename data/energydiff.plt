...
#set y2tics out nomirror scale 0.5 offset -0.5,0
...
do for[i=0:(ix*iy-1)]{
	...
	set xzeroaxis
	plot [:][:] for[j=0:1] "-" index i using (82-$1)/82.:(column(4+j*2)-$2):(column(5+j*2)+$3) w errorbars pt 7 ps 0.5 title word("h-f l-f",j+1)
	unset label
	unset format
}
unset multiplot
