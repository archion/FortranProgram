...
do for[i=0:(ix*iy-1)]{
	...
	plot [0.05:0.39][0:] for[j=0:3] "-" index 0 every :::word("0 6 9",i+1)::word("0 6 9",i+1) using 1:(column(j+2)) with line lw 5 lc 3-j  title " ", for[j=0:3]"-" index 0 every :::word("0 6 9",i+1)::word("0 6 9",i+1) using 1:(column(10+j)>0?column(2+j):1/0):(sprintf("{/=10 %0.4f}",column(1))) with labels notitle
	unset key
	unset label
	unset format
}
