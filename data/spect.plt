...
set ytics 0.2 out nomirror scale 0.5 offset 0.5,0
...
do for[i=0:(ix*iy-1)]{
	...
	#set logscale y
	#plot [0:25][0:1] "-" using -1:($1):(-1*$5) with image title " "
	plot [-1:5][0:] for[j=0:2] "-" every :::j::j using (1./0.3*$1):(-1*$3) with lp lw 5 pt 7 ps 0.3 title ""
	unset label
	unset format
}
unset multiplot
