program rpa
	...
	integer,parameter :: mk=512,mq=128,ms=100,mw=8
	...
	complex(8) :: H(4,4),Uk(4,4),Ukq(4,4),UEk(4),WORK(20),Xq(2,2,0:mq,0:mq,mw),Xq_tmp(2,2),eka,cth,sth,cfy(2),sfy(2)
	integer :: i,j,k,l,kk,ll,m,n,o,p,INFO
						   ...
						   ! Ukq=reshape((/dcmplx(cos(th)*cos(fy(1))),-img*sin(th)*cos(fy(1)),dcmplx(cos(th)*sin(fy(1))),&
							...
							do n=1,4
								do m=1,4
									...
									do p=1,mw
										w=0.05d0*p
										Xq(:,:,k,l,p)=Xq(:,:,k,l,p)+(1-fk(n)-fkq(m))/(w-ek(n)-ekq(m)+img*0.005d0)*Xq_tmp
									enddo
								enddo
							enddo
						enddo
	...
	! (1d0+Jq*(Xq(1,1,:,:)-Xq(2,2,:,:))-Jq**2*(Xq(1,1,:,:)*Xq(2,2,:,:)-Xq(1,2,:,:)*Xq(2,1,:,:))))
	do p=1,mw
		write(filename,"(A12,I3.3,A4)")"../data/rpa_",p,".dat"
		open(unit=10+p,file=filename)
		do k=-mq,mq
			qx=pi*(1d0+0.5d0*k/mq)
			do l=-mq,mq
				qy=pi*(1d0+0.5d0*l/mq)
				...
				kk=abs(k)
				ll=abs(l)
				Xq_RPA=dimag((Xq(1,1,kk,ll,p)-Jq*(Xq(1,1,kk,ll,p)*Xq(2,2,kk,ll,p)+Xq(1,2,kk,ll,p)*Xq(2,1,kk,ll,p)))/&
					(1d0+Jq*(Xq(1,1,kk,ll,p)-Xq(2,2,kk,ll,p))-Jq**2*(Xq(1,1,kk,ll,p)*Xq(2,2,kk,ll,p)-Xq(1,2,kk,ll,p)*&
					Xq(2,1,kk,ll,p))))
				write(10+p,"(4e16.3)")qx,qy,Xq_RPA,dimag(Xq(1,1,kk,ll,p))
			enddo
			write(10+p,"(1x)")
		enddo
	enddo
