module pmt
	...
	real(8), parameter :: t(5)=(/2d0,-0.9d0,0.d0,0.d0,0d0/),&
		!V=0.12d0,DJ=0.35d0,&
		V=0.d0,DJ=1.5d0,&
	Vs=0.5d0*(DJ-V),Vd=0.5d0*DJ+V,alp=0.45!alp=0.34
	real(8) :: it=0.02d0
...
program main
	...
	do n=1,size(nd)
		...
		write(*,"(1X)")
		!check spinsus()
		call spinsus(nf,Tk,pg,sc,ap,sp,q(mq/2+1:mq/2+1,mq/2+1:mq/2+1,:),romg,rVeff(:,1:1,1:1,:,:))
		...
		!enddo
		!do l=1,mo,16
			!call mwrite(20,dreal(Veff(l,:,:,1,1)))
			!write(20,"(1x)")
			!call mwrite(20,dreal(Veff(l,:,:,2,2)))
			!write(20,"(1x)")
			!call mwrite(20,dreal(Veff(l,:,:,1,2)))
			!write(20,"(1x)")
			!call mwrite(20,dreal(Veff(l,:,:,2,1)))
			!write(20,"(1x)")
		!enddo
		!stop
		!do l=1,mq
